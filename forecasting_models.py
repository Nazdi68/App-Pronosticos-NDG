        ...
        for i in range(h_future):
            ...
            if not np.isnan(current_ma_fc) and isinstance(series_for_ma.index, pd.DatetimeIndex) and not series_for_ma.empty:
                ...
                try:
                    ...
                    # Asegurarse de que new_pt tenga el mismo tipo de índice si series_for_ma es una Serie
                    if isinstance(series_for_ma, pd.Series):
                        new_pt = pd.Series([current_ma_fc], index=pd.DatetimeIndex([next_dt]))
                    else: # Si es un DataFrame (no debería ser el caso aquí, pero por si acaso)
                        new_pt = pd.DataFrame({'value': [current_ma_fc]}, index=pd.DatetimeIndex([next_dt]))
                    series_for_ma = pd.concat([series_for_ma, new_pt.squeeze(axis=0 if isinstance(new_pt, pd.DataFrame) and len(new_pt.columns)==1 else None)]) # Squeeze si es DataFrame de una columna
                except Exception: 
    ...
    try:
        ...
        if full_series.empty or len(full_series) < min_samples:
            if h_future > 0:
                ...
                future_fc_output = np.array(future_fc_output_raw) # Asegurar array

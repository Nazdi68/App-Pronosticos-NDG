...
def seasonal_naive_forecast(train_series, test_series, h_future, seasonal_period):
    # ... (código sin cambios de la versión anterior, asegurando manejo de series vacías) ...
    ...
    if not test_series.empty:
        for i in range(len(test_series)):
            ...
            else: test_forecast_values[i] = np.nan 
...
def moving_average_forecast(train_series, test_series, h_future, window):
    """Pronóstico de Promedio Móvil con ventana deslizante."""
    model_name = f"Promedio Móvil (Ventana {window})"
    nan_forecast_future = np.full(h_future, np.nan)

    if window <= 0 or train_series.empty or window > len(train_series):
        return nan_forecast_future, None, np.nan, np.nan, model_name + " (Ventana Inválida o Datos Insuf.)"

    # Evaluación en test set
    test_forecast_values = np.zeros(len(test_series))
    if not test_series.empty:
        # Para la evaluación en test, el promedio móvil se calcula sobre los datos de entrenamiento
        # que preceden a cada punto del test set.
        # Una forma simplificada es usar el MA de los últimos 'window' puntos de train para todo el test.
        # Una forma más correcta (pero más compleja de implementar aquí para out-of-sample puro) sería
        # recalcular el MA iterativamente a medida que se avanza en el test set.
        # Por simplicidad, usaremos el MA del final del train_set para todo el test_set.
        if len(train_series) >= window:
            ma_for_test = train_series.iloc[-window:].mean()
            test_forecast_values = np.full(len(test_series), ma_for_test)
        else: # No suficientes datos para una ventana completa en train
            ma_for_test = train_series.mean() # Usar promedio de lo que haya
            test_forecast_values = np.full(len(test_series), ma_for_test)
    else:
        test_forecast_values = np.array([])
        
    rmse, mae = calculate_metrics(test_series, test_forecast_values)
    
    # Pronóstico futuro
    # Se entrena/calcula sobre la serie completa (train + test)
    full_series = pd.concat([train_series, test_series])
    if full_series.empty or window > len(full_series):
        return nan_forecast_future, None, rmse, mae, model_name + " (Datos Insuf. para Pron. Futuro)"

    future_forecast_values = np.zeros(h_future)
    # Serie temporal que se extiende con los pronósticos para calcular los siguientes MA
    series_for_future_ma = full_series.copy()
    
    for i in range(h_future):
        if len(series_for_future_ma) >= window:
            current_fc = series_for_future_ma.iloc[-window:].mean()
        else: # No suficientes datos históricos para la ventana completa
            current_fc = series_for_future_ma.mean() if not series_for_future_ma.empty else np.nan
        
        future_forecast_values[i] = current_fc
        
        # Añadir el pronóstico actual a la serie para el siguiente paso
        # Necesitamos generar un nuevo índice de tiempo para este punto pronosticado
        if not series_for_future_ma.empty and isinstance(series_for_future_ma.index, pd.DatetimeIndex):
            last_date = series_for_future_ma.index[-1]
            freq_inferred = pd.infer_freq(series_for_future_ma.index)
            if freq_inferred:
                next_date = last_date + pd.tseries.frequencies.to_offset(freq_inferred)
            else: # Fallback si no se infiere frecuencia (ej., usando el delta de los últimos 2 puntos)
                if len(series_for_future_ma.index) > 1:
                    next_date = last_date + (series_for_future_ma.index[-1] - series_for_future_ma.index[-2])
                else: # Si solo hay un punto, no podemos inferir, sumamos un día como fallback extremo
                    next_date = last_date + pd.Timedelta(days=1) 
            
            # Convertir a pd.Series antes de concatenar si es un array numpy
            if isinstance(series_for_future_ma, np.ndarray):
                series_for_future_ma = pd.Series(series_for_future_ma) # Esto necesita un índice

            # Usar pd.concat para añadir el nuevo valor
            new_point = pd.Series([current_fc], index=[next_date])
            series_for_future_ma = pd.concat([series_for_future_ma, new_point])

        else: # Si el índice no es DatetimeIndex o la serie está vacía, no podemos extender
            future_forecast_values[i] = np.nan # Marcar como inválido si no podemos extender
...
def forecast_with_statsmodels(train_series, test_series, h_future, model_name_short, seasonal_period=None, holt_params=None, hw_params=None):
    ...
    if model_name_short == "Holt-Winters" and seasonal_period and seasonal_period > 1:
        min_data_needed = max(min_data_needed, 2 * seasonal_period + 1) 
    ...
    fit_model_train = None; model_display_name = model_name_short
    try:
        if model_name_short == "SES":
            ...
            if not seasonal_period or seasonal_period <= 1: return np.full(h_future, np.nan), None, np.nan, np.nan, "HW: Período estacional > 1 requerido."
        ...
        else: return np.full(h_future, np.nan), None, np.nan, np.nan, f"Modelo Statsmodels '{model_name_short}' no reconocido."
        ...
        if full_series.empty or len(full_series) < min_data_needed:
            ...
            future_ci_df = None
            if h_future > 0:
                try: pred_obj = fit_model_train.get_prediction(start=len(train_series), end=len(train_series) + h_future -1); future_ci_df = pred_obj.conf_int(alpha=0.05)
                except: pass
        ...
        future_ci_df = None
        if h_future > 0:
            try: pred_obj_full = fit_model_full.get_prediction(start=len(full_series), end=len(full_series) + h_future - 1); future_ci_df = pred_obj_full.conf_int(alpha=0.05)
    ...
    except Exception as e: return np.full(h_future, np.nan), None, np.nan, np.nan, f"{model_name_short}: Error ({str(e)[:30]}...)"
...
def forecast_with_auto_arima(train_series, test_series, h_future, seasonal_period, arima_params=None):
    ...
    if train_series.empty or len(train_series) < min_samples:
        ...
        return np.full(h_future, np.nan), None, np.nan, np.nan, msg
    try:
        m_val = seasonal_period if seasonal_period > 1 else 1; seasonal_flag = True if seasonal_period > 1 else False
        ap = arima_params or {}
        ...
        final_model = pm.ARIMA(order=auto_arima_model_train.order, seasonal_order=auto_arima_model_train.seasonal_order if seasonal_flag else (0,0,0,0),suppress_warnings=True).fit(full_series)
    ...
    except Exception as e: return np.full(h_future, np.nan), None, np.nan, np.nan, f"{model_base_name}: Error ({str(e)[:30]}...)"
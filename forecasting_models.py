        ...
        if not test_series.empty:
            for i in range(len(test_series)):
                 if len(train_series) - seasonal_period + (i % seasonal_period) < len(train_series) and \
                    len(train_series) - seasonal_period + (i % seasonal_period) >= 0: # Sin ':' extra
        ...
        for i in range(h_future):
            ...
            if not np.isnan(current_ma_fc) and isinstance(series_for_ma.index, pd.DatetimeIndex) and not series_for_ma.empty:
                ...
                try:
                    ...
                    if isinstance(series_for_ma, pd.Series):
                        new_pt = pd.Series([current_ma_fc], index=pd.DatetimeIndex([next_dt]))
                    else: 
                        new_pt = pd.DataFrame({'value': [current_ma_fc]}, index=pd.DatetimeIndex([next_dt])) # Esto no debería pasar si series_for_ma es una Serie
                    series_for_ma = pd.concat([series_for_ma, new_pt.squeeze(axis=0 if isinstance(new_pt, pd.DataFrame) and len(new_pt.columns)==1 else None)])
                except Exception: 
...
def forecast_with_statsmodels(train_series, test_series, h_future, model_name_short, seasonal_period=None, holt_params=None, hw_params=None):
    ...
    try:
        ...
        if full_series.empty or len(full_series) < min_data_needed:
            ...
            if h_future > 0:
                try: # CORRECCIÓN DE SINTAXIS AQUÍ
                    pred_obj = fit_model_train.get_prediction(start=len(train_series), end=len(train_series) + h_future - 1)
                    future_ci_df_res = pred_obj.conf_int(alpha=0.05)
                except Exception: # Capturar cualquier excepción y continuar
                    pass 
        ...
        if h_future > 0: 
            try: # CORRECCIÓN DE SINTAXIS AQUÍ
                ...
                future_ci_df_res = pred_obj_full.conf_int(alpha=0.05)
            except Exception: # Capturar cualquier excepción y continuar
                future_ci_df_res = None 

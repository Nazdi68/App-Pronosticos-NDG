            ...
            with st.spinner("Calculando modelos... Esto puede tardar unos momentos."):
                ...
                for spec_item_v22_loop in model_execution_list: 
                    try:
                        ...
                        fc_on_test_v22 = None
                # Bucle de ejecución
                st.write(f"DEBUG: Iniciando bucle de ejecución de {len(model_execution_list)} modelos.") # Nuevo DEBUG
                for i_spec, spec_item_v22_loop in enumerate(model_execution_list): 
                    model_name_to_try = spec_item_v22_loop.get('name_override', spec_item_v22_loop['func'].__name__)
                    st.write(f"--- DEBUG: Intentando Modelo {i_spec+1}: {model_name_to_try} ---") # Nuevo DEBUG
                    try:
                        # Estas son las variables que recibe la función de modelo
                        fc_future_v22, ci_future_v22, rmse_v22, mae_v22, name_from_func_v22 = spec_item_v22_loop["func"](*spec_item_v22_loop["args"])
                        name_display_v22 = spec_item_v22_loop["name_override"] or name_from_func_v22
                        
                        st.write(f"DEBUG: Modelo {name_display_v22} ejecutado. RMSE: {rmse_v22}, MAE: {mae_v22}") # Nuevo DEBUG
                        if fc_future_v22 is not None:
                            st.write(f"DEBUG: {name_display_v22} - forecast_future tiene {len(fc_future_v22)} elementos.")
                        else:
                            st.write(f"DEBUG: {name_display_v22} - forecast_future es None.")
                        ...
                        if st.session_state.use_train_test_split and not test_s_for_exec_btn.empty and not any(err in name_display_v22 for err in ["Error","Insuf","Inválido","FALLÓ"]):
                            # ... (TU LÓGICA COMPLETA PARA fc_on_test_v22 DE LA VERSIÓN v3.19/v3.20/v3.21)
                            # Esta sección es crucial y debe estar completa para cada tipo de modelo
                            if spec_item_v22_loop["type"] == "baseline":
                                ...
                                # ... (etc. para otros baselines) ...
                            elif spec_item_v22_loop["type"] == "statsmodels":
                                # ... (lógica de reajuste para statsmodels) ...
                                pass 
                            elif spec_item_v22_loop["type"] == "autoarima":
                                # ... (lógica de reajuste para autoarima) ...
                                pass
                        # --- FIN: Lógica para fc_on_test_v22 ---

                        st.session_state.model_results.append({
                            'name':name_display_v22,
                            'rmse':rmse_v22,
                            'mae':mae_v22,
                            'forecast_future':fc_future_v22,
                            'conf_int_future':ci_future_v22,
                            'forecast_on_test':fc_on_test_v22
                        })
                        st.write(f"--- DEBUG: Modelo {name_display_v22} AÑADIDO a model_results ---") # Nuevo DEBUG
                    except Exception as e_model_v22_loop: 
                        st.warning(f"Error GRAVE procesando {model_name_to_try}: {type(e_model_v22_loop).__name__} - {str(e_model_v22_loop)[:150]}")
                        # Añadir una entrada de error a model_results para saber que falló
                        st.session_state.model_results.append({
                            'name': f"{model_name_to_try} (FALLÓ EJECUCIÓN: {type(e_model_v22_loop).__name__})", 
                            'rmse': np.nan, 'mae': np.nan, 'forecast_future': np.full(h_for_exec_btn, np.nan), # Array de NaNs
                            'conf_int_future': None, 'forecast_on_test': None
                        })
                
                # --- DEBUG: Mostrar contenido de st.session_state.model_results DESPUÉS DEL BUCLE ---
                # (El código de depuración que ya tenías aquí está bien)

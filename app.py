        ...
        if df_processed_for_models_btn is None or target_col_for_models_btn is None or \
           target_col_for_models_btn not in df_processed_for_models_btn.columns: 
            ...
            with st.spinner("Calculando modelos... Esto puede tardar unos momentos."):
                ...
                st.write(f"Horizonte (st.session_state.forecast_horizon) usado para filtrar: {st.session_state.forecast_horizon}")
                st.write("--- FIN DEBUG ---")

            if not st.session_state.model_results: st.error("No se generaron resultados de modelos.")
            valid_results_final_v20_list = [
    r for r in st.session_state.model_results if 
    pd.notna(r.get('rmse')) and 
    r.get('forecast_future') is not None and 
    isinstance(r.get('forecast_future'), (np.ndarray, list)) and 
    len(r.get('forecast_future')) == st.session_state.forecast_horizon # Usar la variable de estado
]

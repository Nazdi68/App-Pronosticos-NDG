...
df_proc_for_tabs_final_render_v22 = st.session_state.get('df_processed')
...
if df_proc_for_tabs_final_v22_render is not None and not df_proc_for_tabs_final_v22_render.empty and \
   ...
   len(model_results_exist_final_v22_render) > 0:
    ...
    tab_rec_v22_render, tab_comp_v22_render, tab_man_v22_render, tab_diag_v22_render = st.tabs(["⭐ Recomendado", "📊 Comparación", "⚙️ Explorar", "💡 Diagnóstico"])
    
    historical_series_for_tabs_v22_render_plot = None
    if target_col_for_tabs_final_v22_render in df_proc_for_tabs_final_v22_render.columns:
        historical_series_for_tabs_v22_render_plot = df_proc_for_tabs_final_v22_render[target_col_for_tabs_final_v22_render]

    with tab_rec_v22_render: # Pestaña Modelo Recomendado
        best_model_rec_v22 = st.session_state.best_model_name_auto
        ...
        elif best_model_rec_v22 and "Error" not in best_model_rec_v22 and "FALLÓ" not in best_model_rec_v22 :
            st.subheader(f"Modelo Recomendado: {best_model_rec_v22}")
            model_data_rec_v22 = next((item for item in st.session_state.model_results if item["name"] == best_model_rec_v22), None)
            if model_data_rec_v22:
                final_df_r_v22, fc_s_r_v22, pi_df_r_v22 = prepare_forecast_display_data(model_data_rec_v22, historical_series_for_tabs_v22_render_plot.index, st.session_state.forecast_horizon)
                if final_df_r_v22 is not None and fc_s_r_v22 is not None and not fc_s_r_v22.empty:
                    # ... (Lógica de visualización y descarga como en v3.19)
                    pass # Placeholder
                else: st.warning(f"No se pudo preparar visualización para '{best_model_rec_v22}'.")
            else: st.info(f"Datos del modelo '{best_model_rec_v22}' no encontrados.")
        else: st.info("No se ha determinado un modelo recomendado. Genere los modelos o verifique errores.")

    with tab_comp_v22_render: # Pestaña Comparación
        st.subheader("Comparación de Modelos")
        # ... (Lógica de visualización como en v3.19)
        pass # Placeholder

    with tab_man_v22_render: # Pestaña Explorar Manualmente
        st.subheader("Explorar Modelo Manualmente")
        # ... (Lógica de visualización como en v3.19)
        pass # Placeholder
        
    with tab_diag_v22_render: # Pestaña Diagnóstico

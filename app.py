...
if df_processed_main_display_v14 is not None and not df_processed_main_display_v14.empty and target_col_main_display_v14:
    ...
    st.session_state.use_train_test_split = st.sidebar.checkbox(
    "Usar Train/Test split", 
    value=st.session_state.use_train_test_split, 
    key="use_split_key_v14", # Asegúrate que la key sea única
    on_change=reset_model_execution_results # Llamar a la función correcta
    )
...
show_results_tabs = False # Variable de control
df_proc_for_tabs_render_check = st.session_state.get('df_processed')
target_col_for_tabs_render_check = st.session_state.get('original_target_column_name')
model_results_exist_render_check = st.session_state.get('model_results')

if df_proc_for_tabs_render_check is not None and \
   not df_proc_for_tabs_render_check.empty and \
   target_col_for_tabs_render_check and \
   model_results_exist_render_check is not None and \
   isinstance(model_results_exist_render_check, list) and \
   len(model_results_exist_render_check) > 0:
    show_results_tabs = True

if show_results_tabs:
    st.header("Resultados del Modelado y Pronóstico")
    tab_rec_final_render, tab_comp_final_render, tab_man_final_render, tab_diag_final_render = st.tabs([
        "⭐ Recomendado", "📊 Comparación", 
        "⚙️ Explorar", "💡 Diagnóstico"
    ])
    
    historical_series_for_tabs_final_render_plot = None
    if target_col_for_tabs_render_check in df_proc_for_tabs_render_check.columns:
        historical_series_for_tabs_final_render_plot = df_proc_for_tabs_render_check[target_col_for_tabs_render_check]

    # --- Pestaña 1: Modelo Recomendado ---
    with tab_rec_final_render:
        best_model_rec_render_val = st.session_state.best_model_name_auto
        if best_model_rec_render_val and "Error" not in best_model_rec_render_val and "FALLÓ" not in best_model_rec_render_val and historical_series_for_tabs_final_render_plot is not None:
            st.subheader(f"Modelo Recomendado: {best_model_rec_render_val}")
            model_data_rec_render_val = next((item for item in st.session_state.model_results if item["name"] == best_model_rec_render_val), None)
            if model_data_rec_render_val:
                final_df_r_render_val, fc_s_r_render_val, pi_df_r_render_val = prepare_forecast_display_data(
                    model_data_rec_render_val, 
                    historical_series_for_tabs_final_render_plot.index, 
                    st.session_state.forecast_horizon
                )
                if final_df_r_render_val is not None and fc_s_r_render_val is not None and not fc_s_r_render_val.empty:
                    if st.session_state.use_train_test_split and st.session_state.test_series_for_plot is not None and not st.session_state.test_series_for_plot.empty and model_data_rec_render_val.get('forecast_on_test') is not None:
                        st.markdown("##### Validación en Test"); 
                        fc_test_r_s_render_val = pd.Series(model_data_rec_render_val['forecast_on_test'], index=st.session_state.test_series_for_plot.index) if isinstance(model_data_rec_render_val['forecast_on_test'],(np.ndarray, list)) and len(model_data_rec_render_val['forecast_on_test'])==len(st.session_state.test_series_for_plot) else model_data_rec_render_val.get('forecast_on_test'); 
                        if fc_test_r_s_render_val is not None: 
                            fig_vr_render_val=visualization.plot_forecast_vs_actual(st.session_state.train_series_for_plot,st.session_state.test_series_for_plot,fc_test_r_s_render_val,best_model_rec_render_val,target_col_for_tabs_render_check); 
                            if fig_vr_render_val: st.pyplot(fig_vr_render_val)
                    st.markdown(f"##### Pronóstico Futuro"); fig_fr_render_val=visualization.plot_final_forecast(historical_series_for_tabs_final_render_plot,fc_s_r_render_val,pi_df_r_render_val,best_model_rec_render_val,target_col_for_tabs_render_check); st.pyplot(fig_fr_render_val) if fig_fr_render_val else st.caption("No se pudo graficar pronóstico.")
                    st.markdown("##### Valores"); st.dataframe(final_df_r_render_val.style.format("{:.2f}")); 
                    dl_key_rec_final_render_btn = f"dl_rec_{best_model_rec_render_val[:15].replace(' ','_').replace('(','').replace(')','').replace(':','_').replace('[','').replace(']','').replace('.','_')}_vF_render_final"
                    st.download_button(f"📥 Descargar ({best_model_rec_render_val})",to_excel(final_df_r_render_val),f"fc_{best_model_rec_render_val.replace(' ','_')}.xlsx",key=dl_key_rec_final_render_btn)
                    st.markdown("##### Recomendaciones"); st.markdown(recommendations.generate_recommendations(best_model_rec_render_val,st.session_state.data_diagnosis_report,True,(pi_df_r_render_val is not None and not pi_df_r_render_val.empty),st.session_state.use_train_test_split and not st.session_state.test_series_for_plot.empty, model_results_list=st.session_state.model_results, target_column_name=target_col_for_tabs_render_check))
                else: st.warning(f"No se pudo preparar visualización para '{best_model_rec_render_val}'.")
            else: st.info(f"Datos del modelo '{best_model_rec_render_val}' no encontrados.")
        elif not historical_series_for_tabs_final_render_plot : st.warning("Datos históricos no disponibles.")
    ...
    with tab_comp_final_render:
        st.subheader("Comparación de Modelos")
        # ... (Tu código completo para la tabla de comparación, que ya parecía estar bien) ...
        metrics_list_comp_final_tab_val = [{'Modelo': r.get('name','N/A'), 'RMSE': r.get('rmse'), 'MAE': r.get('mae')} for r in st.session_state.model_results if pd.notna(r.get('rmse')) and r.get('forecast_future') is not None]
        if metrics_list_comp_final_tab_val:
            metrics_df_comp_final_tab_val = pd.DataFrame(metrics_list_comp_final_tab_val).sort_values(by='RMSE').reset_index(drop=True)
            def highlight_best_final_tab_val(row): return ['background-color: lightgreen' if row.Modelo == st.session_state.best_model_name_auto else ''] * len(row)
            st.dataframe(metrics_df_comp_final_tab_val.style.format({'RMSE':"{:.3f}", 'MAE':"{:.3f}"}).apply(highlight_best_final_tab_val, axis=1))
    ...
    with tab_man_final_render:
        st.subheader("Explorar Modelo Manualmente")
        # ... (Tu código completo para el selector y visualización del modelo manual, que ya parecía estar bien) ...
        valid_manual_models_final_list_tab_val = [r['name'] for r in st.session_state.model_results if r.get('forecast_future') is not None and pd.notna(r.get('rmse'))]
        if valid_manual_models_final_list_tab_val and historical_series_for_tabs_final_render_plot is not None:
            # ... (Lógica completa para el selector y visualización del modelo manual) ...
            pass # Placeholder - Asegúrate de rellenar esto como en la v3.13
        elif not historical_series_for_tabs_final_render_plot : st.warning("Datos históricos no disponibles.")
        else: st.warning("No hay modelos válidos para exploración. Genere los modelos.")
        
    # --- Pestaña 4: Diagnóstico y Guía ---
    with tab_diag_final_render:
...
# --- Mensajes si las pestañas no se muestran (Sección final CORREGIDA) ---
    ...
    # Estas variables se usan para decidir qué mensaje mostrar.
    df_loaded_is_present = st.session_state.get('df_loaded') is not None
    df_processed_value = st.session_state.get('df_processed')
    model_results_list = st.session_state.get('model_results')

    # Comprobar si df_processed_value es un DataFrame y está vacío
    df_processed_is_empty_or_none = (df_processed_value is None or \
                                    (isinstance(df_processed_value, pd.DataFrame) and df_processed_value.empty))
    
    # Comprobar si model_results_list es una lista y está vacía
    model_results_is_empty_or_none = (model_results_list is None or \
                                      (isinstance(model_results_list, list) and not model_results_list))


    if uploaded_file is None and not df_loaded_is_present: 
        st.info("👋 ¡Bienvenido! Cargue un archivo para comenzar.")
    elif df_loaded_is_present and df_processed_is_empty_or_none: 
        st.warning("⚠️ Por favor, aplique preprocesamiento a los datos cargados o verifique el resultado.")
    elif df_loaded_is_present and not df_processed_is_empty_or_none and model_results_is_empty_or_none:
        st.info("Datos preprocesados. Por favor, genere los modelos para ver los resultados.")
    # Puedes añadir un 'else' aquí para un estado inesperado si fuera necesario.

st.sidebar.markdown("---"); st.sidebar.info("Asistente de Pronósticos PRO v3.14")
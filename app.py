    ...
    if st.sidebar.button("📊 Generar y Evaluar Modelos", key="gen_models_btn_key_v20_action"):
        ...
        if df_proc_for_models_btn is None or target_col_for_models_btn is None or \
         ...
         with st.spinner("Calculando modelos... Esto puede tardar unos momentos."):
                # Línea 267 en tu código
                model_execution_list = [] # <--- Nombre de lista consistente
                
                # --- INICIO DE LÍNEAS DE DEPURACIÓN (AÑADIR ESTO) ---
                st.write(f"DEBUG (Justo después de inicializar): Tipo de model_execution_list es {type(model_execution_list)}, Es Lista: {isinstance(model_execution_list, list)}")
                # Suponiendo que train_s_for_run, test_s_for_run, h_for_run están definidas antes de este bloque with st.spinner
                # (lo cual deberían estar, según el código completo que hemos manejado)
                st.write(f"DEBUG: train_s_for_run es None? {train_s_for_run is None}, test_s_for_run es None? {test_s_for_run is None}, h_for_run: {h_for_run if 'h_for_run' in locals() else 'h_for_run no definida en este scope aún'}")
                if 'train_s_for_run' in locals() and train_s_for_run is not None: # locals() para ver si la variable existe
                    st.write(f"DEBUG: train_s_for_run tiene {len(train_s_for_run)} elementos.")
                if 'test_s_for_run' in locals() and test_s_for_run is not None:
                    st.write(f"DEBUG: test_s_for_run tiene {len(test_s_for_run)} elementos.")
                # --- FIN DE LÍNEAS DE DEPURACIÓN ---
                
                # Línea 268 en tu código (comentario)
                # Baselines
                
                # Línea 269/270 en tu código (el primer append)
                try:
                    st.write("DEBUG: Intentando añadir Promedio Histórico...") 
                    ...
                    st.write("DEBUG: Promedio Histórico añadido a la lista de especificaciones.") 
                except AttributeError as ae: # Capturar específicamente el AttributeError
                    st.error(f"DEBUG: AttributeError al intentar AÑADIR Promedio Histórico a la lista: {ae}")
                    st.write(f"DEBUG: Tipo de model_execution_list en error: {type(model_execution_list)}")
                    st.stop() # Detener si el append falla aquí
                except Exception as e_debug_ha:
                    st.error(f"DEBUG: Otro error al intentar AÑADIR Promedio Histórico a la lista: {e_debug_ha}")
                    st.stop() # Detener si hay otro error aquí

                # Para el resto de los appends, puedes añadir try-except similares si el problema persiste
                # o simplemente dejar que fallen si el primero ya dio el AttributeError.
                # Por ahora, nos enfocamos en el primer append.
                ...
                # Statsmodels
                holt_p_exec_btn = {'damped_trend': st.session_state.holt_damped} # Asumo que estas variables _btn son las correctas para este scope
                ...
                if s_period_for_run > 1: stats_configs_btn.append(("Holt-Winters", hw_p_exec_btn))
                
                for name_s_btn_item, params_s_btn_item in stats_configs_btn: # Asumo que estas variables _btn son las correctas
                    model_execution_list.append({"func": forecasting_models.forecast_with_statsmodels, "args": [train_s_for_run, test_s_for_run, h_for_run, name_s_btn_item, s_period_for_run if name_s_btn_item=="Holt-Winters" else None, params_s_btn_item if name_s_btn_item=="Holt" else None, params_s_btn_item if name_s_btn_item=="Holt-Winters" else None], "name_override": None, "type":"statsmodels", "model_short_name": name_s_btn_item, "params_dict": params_s_btn_item})
                ...
                if st.session_state.run_autoarima:
                    ...
                    model_execution_list.append({"func": forecasting_models.forecast_with_auto_arima, "args": [train_s_for_run, test_s_for_run, h_for_run, s_period_for_run, arima_p_btn_item], "name_override": None, "type":"autoarima", "arima_params_dict": arima_p_btn_item})

                # Bucle de ejecución
                # Esta es la línea 285 en tu imagen, y el bucle que procesa model_execution_list
                for spec_item_btn_loop in model_execution_list: 
                    # ... (el resto de tu código para ejecutar los modelos y añadir a st.session_state.model_results)
                    pass # Placeholder para el resto del bucle

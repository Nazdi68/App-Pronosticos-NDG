    ...
    if st.sidebar.button("📊 Generar y Evaluar Modelos", key="gen_models_btn_key_v19_action"):
        ...
        if df_processed_for_models is None or target_col_for_models is None or \
           ...
           with st.spinner("Calculando modelos... Esto puede tardar unos momentos."):
                ...
                st.write(f"DEBUG: train_s_for_run es None? {train_s_for_run is None}, test_s_for_run es None? {test_s_for_run is None}, h_for_run: {h_for_run}")
                if train_s_for_run is not None:
                    st.write(f"DEBUG: train_s_for_run tiene {len(train_s_for_run)} elementos.")
                if test_s_for_run is not None:
                    st.write(f"DEBUG: test_s_for_run tiene {len(test_s_for_run)} elementos.")
                # --- FIN DE LÍNEAS DE DEPURACIÓN ---

                # Baselines
                try:
                    st.write("DEBUG: Intentando añadir Promedio Histórico...") # Debug
                    ...
                    st.write("DEBUG: Promedio Histórico añadido a la lista de especificaciones.") # Debug
                except Exception as e_debug_ha:
                    st.error(f"DEBUG: Error al intentar AÑADIR Promedio Histórico a la lista: {e_debug_ha}")
                
                try:
                    st.write("DEBUG: Intentando añadir Naive...") # Debug
                    ...
                    st.write("DEBUG: Naive añadido a la lista de especificaciones.") # Debug
                except Exception as e_debug_naive:
                    st.error(f"DEBUG: Error al intentar AÑADIR Naive a la lista: {e_debug_naive}")

                try:
                    st.write("DEBUG: Intentando añadir Promedio Móvil...") # Debug
                    ...
                    st.write("DEBUG: Promedio Móvil añadido a la lista de especificaciones.") # Debug
                except Exception as e_debug_ma:
                    st.error(f"DEBUG: Error al intentar AÑADIR Promedio Móvil a la lista: {e_debug_ma}")

                if s_period_for_run > 1: 
                    try:
                        st.write("DEBUG: Intentando añadir Estacional Ingénuo...") # Debug
                        model_execution_list.append({"func": forecasting_models.seasonal_naive_forecast, "args": [train_s_for_run, test_s_for_run, h_for_run, s_period_for_run], "name_override": None, "type":"baseline"})
                        st.write("DEBUG: Estacional Ingénuo añadido a la lista de especificaciones.") # Debug
                    except Exception as e_debug_sn:
                        st.error(f"DEBUG: Error al intentar AÑADIR Estacional Ingénuo a la lista: {e_debug_sn}")
                
                # Statsmodels (DEJA EL RESTO DE LA LÓGICA PARA Statsmodels y AutoARIMA COMO ESTABA)
